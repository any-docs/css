...
The **`<shape>`** [CSS](/en-US/docs/Web/CSS) [data type](/reference/values/Data_types) defines the specific form (shape) of a region. The region represents the part of an element to which the {{cssxref("clip")}} property applies.
...
When animated, values of the `<shape>` data type are interpolated over their `top`, `right`, `bottom`, and `left` components, each treated as a real, floating-point number. The speed of the interpolation is determined by the [easing function](/reference/values/easing-function) associated with the animation.
