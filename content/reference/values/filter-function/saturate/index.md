...
The **`saturate()`** [CSS](/en-US/docs/Web/CSS) [function](/reference/values/Functions) super-saturates or desaturates the input image. Its result is a `filter-function`.
...
- `amount` <Badge type="info" text="Optional" />
  - The amount of the conversion, specified as a `&lt;number&gt;` or a `&lt;percentage&gt;`. A value under `100%` desaturates the image, while a value over `100%` super-saturates it. A value of `0%` is completely unsaturated, while a value of `100%` leaves the input unchanged. The initial value for {{Glossary("interpolation")}} is `1`. The default value is `1`.
