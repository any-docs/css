...
The **`<calc-sum>`** [CSS](/en-US/docs/Web/CSS) [data type](/reference/values/Data_types) represents an expression which performs a calculation in any [CSS math function](/reference/values/Functions#math_functions). The expression executes a basic arithmetic operation of addition and subtraction between two values.
...
Including [CSS variables](/guides/Cascading_variables) in `calc-sum` expressions is also allowed. The following code `calc(10px + var(--variable))`, is a valid expression.
