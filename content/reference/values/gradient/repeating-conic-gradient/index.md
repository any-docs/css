...
The **`repeating-conic-gradient()`** [CSS](/en-US/docs/Web/CSS) [function](/reference/values/Functions) creates an image consisting of a repeating gradient (rather than a [single gradient](/reference/values/gradient/conic-gradient)) with color transitions rotated around a center point (rather than [radiating from the center](/reference/values/gradient/repeating-radial-gradient)).
...
> [!NOTE]
> Rendering of color stops in repeating conic gradients follows the same rules as [color stops in linear gradients](/reference/values/gradient/linear-gradient#composition_of_a_linear_gradient).
