...
The **`repeat()`** [CSS](/en-US/docs/Web/CSS) [function](/reference/values/Functions) represents a repeated fragment of the [track list](/guides/Grid_layout/Basic_concepts), allowing a large number of columns or rows that exhibit a recurring pattern to be written in a more compact form.
...
- **tracks**: the second argument specifies the set of tracks that will be repeated. Fundamentally this consists of one or more values, where each value represents the size of that track. Each size is specified using either a [`<track-size>`](#track-size) value or a [`<fixed-size>`](#fixed-size) value. You can also specify one or more [line names](/guides/Grid_layout/Named_grid_lines) before or after each track, by providing [`<line-names>`](#line-names) values before and/or after the track size.
...
There is a fourth form, `<name-repeat>`, which is used to add line names to subgrids. It only used with the [`subgrid`](/guides/Grid_layout/Subgrid) keyword and only specifies line names, not track sizes.
...
- `<fixed-size>`
  - One of the following forms:
    ...
    - a `minmax()` function with:
      ...
      - `max` given as one of a `&lt;length-percentage&gt;` value, a `&lt;flex&gt;` value, or one of the following keywords: [`min-content`](#min-content), [`max-content`](#max-content), or [`auto`](#auto)
    - a `minmax()` function with:
      - `min` given as a `&lt;length-percentage&gt;` value or one of the following keywords: [`min-content`](#min-content), [`max-content`](#max-content), or [`auto`](#auto)
...
- `<track-size>`
  - One of the following forms:
    - a `&lt;length-percentage&gt;` value, a `&lt;flex&gt;` value, or one of the following keywords: [`min-content`](#min-content), [`max-content`](#max-content), or [`auto`](#auto)
    - a `minmax()` function with:
      - `min` given as a `&lt;length-percentage&gt;` value, or one of the following keywords: [`min-content`](#min-content), [`max-content`](#max-content), or [`auto`](#auto)
      - `max` given as a `&lt;length-percentage&gt;` value, a `&lt;flex&gt;` value, or one of the following keywords: [`min-content`](#min-content), [`max-content`](#max-content), or [`auto`](#auto)
