...
For more examples, see our [Using CSS custom functions](/guides/Custom_functions_and_mixins/Using_custom_functions) guide.
...
In our styles, we first define the CSS custom function. The function is called `--transparent` and accepts two parameters: a color and a numeric alpha channel value. Inside the function body, we use [relative color syntax](/guides/Colors/Using_relative_colors) to transform the passed color into an [`oklch()`](/reference/values/color_value/oklch) color with an alpha channel equal to the passed alpha value; this becomes the function's `result`:
