...
The **`<image>`** [CSS](/en-US/docs/Web/CSS) [data type](/reference/values/Data_types) represents a two-dimensional image.
...
- Generated by a [paint worklet](/en-US/docs/Web/API/CSS_Painting_API) with the `paint()` function.
...
CSS determines an object's _concrete size_ using (1) its _intrinsic dimensions_; (2) its _specified size_, defined by CSS properties like `width`, `height`, or `background-size`; and (3) its _default size_, determined by the kind of property the image is used with:
