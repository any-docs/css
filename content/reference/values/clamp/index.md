...
In all three cases, the page uses a combination of a viewport-relative units ([`vw`](/reference/values/length#vw) and {{cssxref("percentage")}}), to set a size that varies with the viewport width, and a value that is not viewport relative ([`rem`](/reference/values/length#rem) and [`px`](/reference/values/length#px)) to implement minimum and/or maximum sizes.
