...
The `progress()` function outputs unitless values, therefore it can be used for the purpose of removing units from values in the same manner as the [`tan(atan2())` hack](https://dev.to/janeori/css-type-casting-to-numeric-tanatan2-scalars-582j). Note however that, due to the updates in behavior around [CSS typed arithmetic](/guides/Values_and_units/Using_typed_arithmetic), this can also be achieved via simple division.
...
This can result in some complex expressions. For example, here we are calculating the first two channels of an [`rgb()`](/reference/values/color_value/rgb) color proportionate to the same width proportion as before:
