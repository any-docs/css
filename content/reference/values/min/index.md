...
The **`min()`** [CSS](/en-US/docs/Web/CSS) [function](/reference/values/Functions) lets you set the smallest (most negative) value from a list of comma-separated expressions as the value of a CSS property value. The `min()` function can be used anywhere a `&lt;length&gt;`, `&lt;frequency&gt;`, `angle`, `&lt;time&gt;`, `&lt;percentage&gt;`, `&lt;number&gt;`, or `&lt;integer&gt;` is allowed.
