...
- A nested `calc-size()` value. This isn't something you'd be likely to do very often, but it is available ensuring using a [CSS variable](/guides/Cascading_variables/Using_custom_properties) as the `<calc-size-basis>` will always work, provided the variable is a valid value for the property `calc-size()` is being set on. So for example, this will work:
...
For example, you could use a [transition](/guides/Transitions) to animate a container `width` between `0` and `auto` like so:
...
In the CSS, we use [flexbox](/guides/Flexible_box_layout) to center the child elements inside the `<section>`, and set the `width` and `height` of the `<section>` to `calc-size()` functions. The `width` is set equal to `fit-content` plus `6rem`. The `height` is set to `auto` multiplied by two.
...
As mentioned earlier, it is possible to use another function inside `calc-size()`. This example sets [`field-sizing: content`](/reference/properties/field-sizing) on {{htmlelement("input")}} elements to make them as wide as the entered content, and then uses a [`max()`](/reference/values/max) function inside `calc-size()` to ensure that the `<input>`s are at least a minimum size, and only start to grow when the entered text becomes wider than that size — by being set to `fit-content` plus `20px`.
