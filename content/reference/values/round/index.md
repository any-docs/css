...
The **`round()`** [CSS](/en-US/docs/Web/CSS) [function](/reference/values/Functions) returns a rounded number based on a selected rounding strategy.

Authors should use a [custom CSS property](/reference/properties/--*) (e.g., `--my-property`) for the rounding value, interval, or both; using the `round()` function is redundant if these have known values.
...
- `valueToRound`
  - The value to be rounded.
    This must be a `&lt;number&gt;`, `&lt;dimension&gt;`, or `&lt;percentage&gt;`, or a mathematical expression that resolves to one of those values.

- `roundingInterval`
  - The rounding interval.
    This is a `&lt;number&gt;`, `&lt;dimension&gt;`, or `&lt;percentage&gt;`, or a mathematical expression that resolves to one of those values. If `valueToRound` is a `&lt;number&gt;`, `roundingInterval` may be omitted and defaults to `1`. Otherwise, omitting it results in an invalid expression.
...
Note that we apply a [custom CSS property](/reference/properties/--*) named `--rounding-interval`, that we will use for the rounding interval.
