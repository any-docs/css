...
The `:is()` pseudo-class requires a [selector list](/guides/Selectors/Selector_structure#selector_list), a comma-separated list of one or more selectors as its argument. The list must not contain a [pseudo-element](/en-US/docs/Web/CSS/Reference/Selectors/Pseudo-elements), but any other simple, compound, and complex selectors are allowed.
...
- {{cssxref(":where()")}} - Like `:is()`, but with 0 [specificity](/guides/Cascade/Specificity).
