...
An invalid selector represents, and therefore matches, nothing. When a selector list contains an invalid selector, the entire style block is ignored, except for the `:is()` and `:where()` pseudo-classes that accept [forgiving selector lists](#forgiving_selector_list).
...
A way to remedy the [invalid selector list](#invalid_selector_list) problem is to use the `:is()` or the `:where()` pseudo-class, which accept a forgiving selector list. Each selector in a forgiving selector list is parsed individually. So any invalid selectors in the list are ignored and the valid ones are used.
...
A relative selector list is a comma-separated selector list parsed as [relative selectors](/guides/Selectors/Selector_structure#relative_selector), which begin with an explicit or implied combinator.
...
- [Selectors and combinators](/guides/Selectors/Selectors_and_combinators)
