...
See [Creating CSS carousels](/guides/Overflow/Carousels) for more carousel examples.
...
We don't need to set [alternative text](/reference/properties/content#alternative_text_string_counter) for the icons on the `content` as the browser takes care of providing appropriate {{glossary("accessible name", "accessible names")}} automatically.
...
The previous example works, but the buttons are not ideally placed. In this section, we will add some CSS to position them using [anchor positioning](/guides/Anchor_positioning).
