...
The **`transition-timing-function`** [CSS](/en-US/docs/Web/CSS) property sets how intermediate values are calculated for CSS properties being affected by a [transition effect](/guides/Transitions/Using).
...
- `<easing-function>`
  - Each `easing-function` represents the easing function to link to the corresponding property to transition, as defined in {{ cssxref("transition-property") }}.
