...
This property is a shorthand for all CSS properties except for `unicode-bidi`, `direction`, and [custom properties](/guides/Cascading_variables/Using_custom_properties).
...
- `initial`
  - Specifies that all the element's properties should be changed to their [initial values](/guides/Cascade/Property_value_processing#initial_value).
- `inherit`
  - Specifies that all the element's properties should be changed to their [inherited values](/guides/Cascade/Inheritance).
- `unset`
  - Specifies that all the element's properties should be changed to their inherited values if they inherit by default, or to their initial values if not.
- `revert`
  - Specifies behavior that depends on the stylesheet origin to which the declaration belongs:
    - If the rule belongs to the [author origin](/guides/Cascade/Introduction#author_stylesheets), the `revert` value rolls back the [cascade](/guides/Cascade/Introduction) to the user level, so that the [specified values](/guides/Cascade/Property_value_processing#specified_value) are calculated as if no author-level rules were specified for the element. For purposes of `revert`, the author origin includes the Override and Animation origins.
    - If the rule belongs to the [user origin](/guides/Cascade/Introduction#user_stylesheets), the `revert` value rolls back the [cascade](/guides/Cascade/Introduction) to the user-agent level, so that the [specified values](/guides/Cascade/Property_value_processing#specified_value) are calculated as if no author-level or user-level rules were specified for the element.
    - If the rule belongs to the [user-agent origin](/guides/Cascade/Introduction#user-agent_stylesheets), the `revert` value acts like `unset`.
- `revert-layer`
...
With the `all` property set to `initial` in the `blockquote` rule, the {{HTMLElement("blockquote")}} element doesn't use the browser default styling anymore: it is an _inline_ element now (initial value), its [`background-color`](/reference/properties/background-color#formal_definition) is `transparent` (initial value), its [`font-size`](/reference/properties/font-size#formal_definition) is `medium`, and its [`color`](/reference/properties/color#formal_definition) is `black` (initial value).
...
When the `unset` value is applied to the `all` property in the `blockquote` rule, the {{HTMLElement("blockquote")}} element doesn't use the browser default styling. Because [`background-color`](/reference/properties/background-color#formal_definition) is a non-inherited property and [`font-size`](/reference/properties/font-size#formal_definition) and [`color`](/reference/properties/color#formal_definition) are inherited properties, the `<blockquote>` element is an _inline_ element now (initial value), its `background-color` is `transparent` (initial value), but its `font-size` is still `small` (inherited value), and its `color` is `blue` (inherited value).
