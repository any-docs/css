...
This property applies to all content, including pseudo-elements, with the exception `::marker` pseudo-elements constructed via a `@counter-style` with a defined [`speak-as`](/en-US/docs/Web/CSS/Reference/At-rules/@counter-style/speak-as) descriptor, which takes precedence over any inherited `speak-as` property value.
