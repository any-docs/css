...
Common choices for operating system color schemes are "light" and "dark", or "day mode" and "night mode". When a user selects one of these color schemes, the operating system makes adjustments to the user interface. This includes [form controls](/en-US/docs/Learn_web_development/Extensions/Forms), [scrollbars](/guides/Scrollbars_styling), and the used values of [CSS system colors](/reference/values/system-color).
...
Alternatively, use the [`light-dark()`](/reference/values/color_value/light-dark) [`<color>` function](/reference/values/Functions#color_functions) to set the foreground and background colors for the different color schemes using a more compact code structure:
