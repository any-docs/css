...
The scroll progress timeline is progressed through by scrolling the scroller horizontally or vertically, depending on the `<axis>` and [writing mode](/guides/Writing_modes/Writing_mode_systems). If the indicated axis does not contain a scrollbar, then the animation timeline will be inactive.
