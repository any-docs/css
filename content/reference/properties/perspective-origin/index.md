...
The **`perspective-origin`** and {{cssxref('perspective')}} properties are attached to the parent of a child transformed in 3-dimensional space, unlike the [`perspective()`](/reference/values/transform-function/perspective) transform function which is placed on the element being transformed.
...
An example showing how to change `perspective-origin` is given in [Using CSS transforms > Changing the perspective origin](/guides/Transforms/Using#changing_the_perspective_origin).
...
- {{cssxref('perspective')}}
- [`transform: perspective()`](/reference/values/transform-function/perspective) function
