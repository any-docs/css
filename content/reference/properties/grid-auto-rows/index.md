...
If a grid item is positioned into a row that is not explicitly sized by `grid-template-rows`, implicit {{glossary("grid", "grid")}} tracks are created to hold it. This can happen either by explicitly positioning into a row that is out of range, or by the auto-placement algorithm creating additional rows.
...
- `grid-auto-flow`
- `grid`
- [Auto-placement in grid layout - sizing rows in the implicit grid](/guides/Grid_layout/Auto-placement#sizing_rows_in_the_implicit_grid)
