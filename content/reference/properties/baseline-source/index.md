...
The **`baseline-source`** [CSS](/en-US/docs/Web/CSS) property defines which [baseline](/reference/values/baseline-position) to use when inline-level boxes have multiple possible baselines, such as multi-line [inline blocks](/guides/Display/Block_and_inline_layout) or inline [flex containers](/guides/Flexible_box_layout/Basic_concepts#the_flex_container).
...
- `auto`
  - Specifies [`last baseline`](/reference/values/baseline-position#last_baseline) alignment for inline-block, [`first baseline`](/reference/values/baseline-position#first_baseline) alignment for everything else.
- `first`
