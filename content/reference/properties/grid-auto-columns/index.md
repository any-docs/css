...
If a grid item is positioned into a column that is not explicitly sized by `grid-template-columns`, implicit {{glossary("grid", "grid")}} tracks are created to hold it. This can happen either by explicitly positioning into a column that is out of range, or by the auto-placement algorithm creating additional columns.
