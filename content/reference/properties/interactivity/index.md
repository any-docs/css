...
This example shows horizontally-scrolling paginated content, with each page snapped to using [CSS Scroll Snap](/guides/Scroll_snap), and the inertness controlled via a [scroll-driven animation](/guides/Scroll-driven_animations) that uses a [view progress timeline](/reference/properties/view-timeline-name). Content displayed in the {{glossary("scroll container")}} is interactive; it becomes inert when it moves out into the overflowing content.
