...
- The three rows are represented by the [physical values](/reference/values/position-area_value#physical_grid_keywords) `top`, `center`, and `bottom`. They also have [logical equivalents](/reference/values/position-area_value#logical_grid_keywords) such as `block-start`, `center`, and `block-end`, and [coordinate equivalents](/reference/values/position-area_value#coordinate_grid_keywords) — `y-start`, `center`, and `y-end`.
...
The dimensions of the center tile are defined by the [containing block](/guides/Display/Containing_block) of the anchor element, while the dimensions of the grid's outer edge are defined by the positioned element's containing block.

The [`<position-area>`](/reference/values/position-area_value) value is composed of one or two keywords, which define the region of the grid the positioned element should be placed inside. To be exact, the containing block of the positioned element is set to the grid area.
...
For detailed information on anchor features, usage, and the `position-area` property, see the [CSS anchor positioning](/guides/Anchor_positioning) module landing page and the [Using CSS anchor positioning](/guides/Anchor_positioning/Using) guide, specifically the section on [setting a `position-area`](/guides/Anchor_positioning/Using#setting_a_position-area).

### Adjusted default behavior

When a [`<position-area>`](/reference/values/position-area_value) value is set on a positioned element, some of its properties will have their default behavior adjusted to provide a good default alignment.
