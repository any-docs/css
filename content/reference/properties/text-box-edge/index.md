...
In this example, we demonstrate the effect of both of these common values, on two {{htmlelement("p")}} elements. Additionally, a `text-box-trim` value of `trim-both` has been set on both of them, so that their start _and_ end edges are trimmed.
...
For a complete interactive `text-box-edge` example, see the [`text-box-trim` page](/reference/properties/text-box-trim#interactive_text-box-trim_and_text-box-edge_value_comparison).
