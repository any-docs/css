...
The **`display`** [CSS](/en-US/docs/Web/CSS) property sets whether an element is treated as a [block or inline box](/guides/Display/Flow_layout) and the layout used for its children, such as [flow layout](/guides/Display/Flow_layout), [grid](/guides/Grid_layout) or [flex](/guides/Flexible_box_layout).

Formally, the **`display`** property sets an element's inner and outer _display types_. The outer type sets an element's participation in [flow layout](/guides/Display/Flow_layout); the inner type sets the layout of children. Some values of `display` are fully defined in their own individual specifications; for example the detail of what happens when `display: flex` is declared is defined in the CSS Flexible Box Model specification.
...
> ```
>
> See [Using the multi-keyword syntax with CSS display](/guides/Display/Multi-keyword_syntax) for more information.
...
- `&lt;display-inside&gt;`
  - These keywords specify the element's inner display type, which defines the type of formatting context that its contents are laid out in (assuming it is a non-replaced element). When one of these keywords is used by itself as a single value, the element's outer display type defaults to `block` (with the exception of `ruby`, which defaults to `inline`).
    ...
    - `flex`
      - The element behaves like a block-level element and lays out its content according to the [flexbox model](/guides/Flexible_box_layout).
    - `grid`
      - The element behaves like a block-level element and lays out its content according to the [grid model](/guides/Grid_layout/Basic_concepts).
    - `ruby`
...
The [CSS display module](/guides/Display) describes a multi-keyword syntax for values you can use with the `display` property to explicitly define **outer** and **inner** display.
...
For more information on these changes, see the [Using the multi-keyword syntax with CSS display](/guides/Display/Multi-keyword_syntax) guide.
...
- [Using the multi-keyword syntax with CSS display](/guides/Display/Multi-keyword_syntax)

### CSS Flow Layout (display: block, display: inline)

- [Block and inline layout in normal flow](/guides/Display/Block_and_inline_layout)
- [Flow layout and overflow](/guides/Display/Flow_layout_and_overflow)
...
- [In flow and out of flow](/guides/Display/In_flow_and_out_of_flow)
...
- [Ordering flex items](/guides/Flexible_box_layout/Ordering_items)
- [Relationship of flexbox to other layout methods](/guides/Flexible_box_layout/Relationship_with_other_layout_methods)
- [Typical use cases of flexbox](/guides/Flexible_box_layout/Use_cases)

### display: grid

- [Basic concepts of grid layout](/guides/Grid_layout/Basic_concepts)
- [Relationship to other layout methods](/guides/Grid_layout/Relationship_with_other_layout_methods)
- [Line-based placement](/guides/Grid_layout/Line-based_placement)
- [Grid template areas](/guides/Grid_layout/Grid_template_areas)
- [Layout using named grid lines](/guides/Grid_layout/Named_grid_lines)
- [Auto-placement in grid layout](/guides/Grid_layout/Auto-placement)
- [Aligning items in CSS grid layout](/guides/Grid_layout/Box_alignment)
- [Grids, logical values and writing modes](/guides/Grid_layout/Logical_values_and_writing_modes)
- [CSS grid layout and accessibility](/guides/Grid_layout/Accessibility)
- [Realizing common layouts using grids](/guides/Grid_layout/Common_grid_layouts)

### Animating display

[Supporting browsers](#browser_compatibility) animate `display` with a [discrete animation type](/guides/Animations/Animatable_properties#discrete). This generally means that the property will flip between two values 50% through animating between the two.
