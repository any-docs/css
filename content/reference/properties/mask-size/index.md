...
The property values is displayed using [generated content](/guides/Generated_content).
...
With `auto`, the star is displayed at its intrinsic `100px` by `100px` size. With `cover`, the star grows to be `400px` tall, covering the entire origin box. With `contain`, the star grows until one dimension equals the same dimension of the [origin box](/reference/properties/mask-origin), meaning that the star is as large as it can be (`200px` wide) but still contained by it.
