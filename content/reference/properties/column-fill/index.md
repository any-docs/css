...
- `balance`
  - Content is equally divided between columns. In fragmented contexts, such as [paged media](/guides/Paged_media), only the last fragment is balanced. Therefore in paged media, only the last page would be balanced.

The specification defines a `balance-all` value, in which content is equally divided between columns in fragmented contexts, such as [paged media](/guides/Paged_media). This value is not yet supported in any browser.
