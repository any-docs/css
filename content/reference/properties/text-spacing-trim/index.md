...
- `<spacing-trim>`
  - Defines the different spacing trim options. Available values are:
    ...
    > [!NOTE]
    > The [CSS Text](/guides/Text) module also defines `trim-both`, `trim-all`, and `auto` values. However, these are not currently implemented in any browser.
...
- `text-autospace`
- [`ic`](/guides/Values_and_units/Numeric_data_types#local_font-relative_lengths) and [`ric`](/guides/Values_and_units/Numeric_data_types#root_font-relative_lengths) units
