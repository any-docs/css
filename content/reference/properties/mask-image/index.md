...
An `<image>` can be any type of image, including generated images such as [CSS gradients](/reference/values/gradient).
...
In this example, we use an `<image>` value as a mask, defining a CSS [radial gradient](/reference/values/gradient/radial-gradient) as our mask image to create a round image with a soft edge.
