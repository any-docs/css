...
- [Size](/guides/Containment/Container_size_and_style_queries): Enable selectively applying CSS rules to a container's children based on a general size or inline size condition such as a maximum or minimum dimension, aspect ratio, or orientation.
- [Scroll-state](/guides/Conditional_rules/Container_scroll-state_queries): Enable selectively applying CSS rules to a container's children based on a scroll-state condition such as whether the container is a scroll container that is partially scrolled or whether the container is a [snap target](/en-US/docs/Glossary/Scroll_snap#snap_target) that is going to be snapped to its scroll snap container.
...
- `inline-size`
  - Establishes a query container for dimensional queries on the [inline axis](/guides/Logical_properties_and_values/Basic_concepts#block_and_inline_dimensions) of the container.
    Applies [style](/reference/properties/contain#style) and [inline-size](/reference/properties/contain#inline-size) containment to the element. The inline size of the element can be [computed in isolation](/guides/Containment/Using#size_containment), ignoring the child elements (see [Using CSS containment](/guides/Containment/Using)).
...
- `size`
  - Establishes a query container for container size queries in both the [inline and block](/guides/Logical_properties_and_values/Basic_concepts#block_and_inline_dimensions) dimensions.
    Applies [style](/reference/properties/contain#style) and [size](/reference/properties/contain#size) containment to the element. Size containment is applied to the element in both the inline and block directions. The size of the element can be computed in isolation, ignoring the child elements.
...
[Container size queries](/guides/Containment/Container_size_and_style_queries#container_size_queries) allow you to selectively apply CSS rules to a container's descendants based on a size condition such as a maximum or minimum dimension, aspect ratio, or orientation.
...
[Container scroll-state queries](/guides/Conditional_rules/Container_scroll-state_queries) allow you to selectively apply CSS rules to a container's children based on a scroll-state condition such as:
...
- Whether the container is positioned via [`position: sticky`](/reference/properties/display) and stuck to a boundary of a {{glossary("scroll container", "scrolling container")}}.
...
The following uses the `inline-size` value to create a containment context for the [inline axis](/guides/Logical_properties_and_values/Basic_concepts#block_and_inline_dimensions) of the container:
