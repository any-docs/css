...
The **`@container`** [CSS](/en-US/docs/Web/CSS) [at-rule](/guides/Syntax/At-rules) is a conditional group rule that applies styles to a [containment context](/guides/Containment/Container_queries#naming_containment_contexts).
...
- `snapped`
  - Queries whether the container is going to be snapped to a [scroll snap](/guides/Scroll_snap) container ancestor along the given axis. Valid `snapped` values include the following keywords:
    - `none`
...
Container queries can also evaluate the computed style of the container element. A _container style query_ is a `@container` query that uses one or more `style()` functional notations. The boolean syntax and logic combining style features into a style query is the same as for [CSS feature queries](/guides/Conditional_rules/Using_feature_queries).
...
The parameter of each `style()` is a single `<style-feature>`. A **`<style-feature>`** is a valid CSS [declaration](/guides/Syntax/Introduction#css_declarations), a CSS property, or a [`<custom-property-name>`](/reference/values/var#values).
...
The following container query checks if the [computed value](/guides/Cascade/Property_value_processing#computed_value) of the container element's `--accent-color` is `blue`:
...
See [Using container scroll-state queries](/guides/Conditional_rules/Container_scroll-state_queries) for full walkthroughs of scroll-state query examples.
