...
People experiencing low vision conditions may have difficulty reading text set with a `font-weight` value of `100` (Thin/Hairline) or `200` (Extra Light), especially if the font has a [low contrast color ratio](/reference/properties/color#accessibility).
...
The CSS [cascade](/guides/Cascade/Introduction) ensures that the three latter declarations override parts of the range that was set in the `FiraSans-Regular` declaration.
