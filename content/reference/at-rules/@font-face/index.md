...
The **`@font-face`** [CSS](/en-US/docs/Web/CSS) [at-rule](/guides/Syntax/At-rules) specifies a custom font with which to display text; the font can be loaded from either a remote server or a locally-installed font on the user's own computer.
...
The `@font-face` at-rule may be used not only at the top level of a CSS, but also inside any [CSS conditional-group at-rule](/guides/Conditional_rules#at-rules_and_descriptors).
