...
The **`font-variation-settings`** [CSS](/en-US/docs/Web/CSS) descriptor allows authors to specify low-level OpenType or TrueType font variations in the `@font-face` at-rule. The values for this descriptor are the same as the `font-variation-settings` property, except for the global keyword values.
...
- `<string> <number>`
  - When rendering text, the list of OpenType axis names is passed to the text layout engine to enable or disable font features. Each setting is always a `&lt;string&gt;` of 4 {{Glossary("ASCII")}} characters, followed by a `number` indicating the axis value. If the `<string>` has more or fewer characters or contains characters outside the U+20 - U+7E code point range, the whole property is invalid. The `<number>` can be fractional or negative.
