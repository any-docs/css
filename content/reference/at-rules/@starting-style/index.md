...
The **`@starting-style`** [CSS](/en-US/docs/Web/CSS) [at-rule](/guides/Syntax/At-rules) is used to define starting values for properties set on an element that you want to transition from when the element receives its first style update, i.e., when an element is first displayed on a previously loaded page.
...
> [!NOTE]
> `@starting-style` is only relevant to CSS transitions. When using [CSS animations](/guides/Animations) to implement such effects, `@starting-style` is not needed. See [Using CSS animations](/guides/Animations/Using) for an example.
