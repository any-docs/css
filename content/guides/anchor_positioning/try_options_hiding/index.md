...
> [!NOTE]
> For information on the basic fundamentals of CSS anchor positioning, see [Using CSS anchor positioning](/guides/Anchor_positioning/Using).
...
The predefined fallback option values of the `position-try-fallbacks` property (defined as [`<try-tactic>`](/reference/properties/position-try-fallbacks#try-tactic)s in the spec) will "flip" the position of the anchor-positioned element across one or both axes if the element would otherwise overflow.
