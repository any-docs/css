...
We're going to create this layout using the _named template areas_ that we learned about in the [Grid template areas](/guides/Grid_layout/Grid_template_areas) guide.
...
As described in the [using named grid lines guide](/guides/Grid_layout/Named_grid_lines), we are using the named lines to place our items. As we have 12 lines all with the same name, we use the name and the index of the line. If you prefer, you can use the line index itself and avoid using named lines.
...
Many layouts are essentially sets of "cards" – product listings, image galleries, and so on. A grid enables creating these listings in a way that is responsive without needing to add [media queries](/guides/Media_queries). In this example, we combine CSS grid and flexbox layouts to make a basic product listing layout.
...
- [CSS grid layout](/guides/Grid_layout)
