...
CSS grid layout and [CSS box alignment](/guides/Box_alignment) features enable us to control how the items align once placed into a grid area, and how the entire grid is aligned.
...
Tracks can be defined using any length unit. Grid also introduces an additional length unit to help us create flexible grid tracks. The [`fr`](/reference/values/flex_value) unit represents a fraction of the available space in the grid container. The next grid definition would create three equal width tracks that grow and shrink according to the available space.
...
Large grids with many tracks can use the [`repeat()`](/reference/values/repeat) notation, to repeat all or a section of the list of grid tracks. For example the grid definition:
...
In this overview, we took a very quick look at the possibilities of grid layouts. Explore and play with the code examples, and then move on to the guide, [relationship of grid layout with other layout methods](/guides/Grid_layout/Relationship_with_other_layout_methods), where we will really start to dig into the details of CSS grid layout.
