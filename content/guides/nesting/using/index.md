...
When using [compound selectors](/guides/Selectors/Selector_structure#compound_selector) in nested CSS you **have** to use the `&` nesting selector. This is because the browser will automatically add whitespace between selectors that do not use the `&` nesting selector.
...
Styles for the `.notices` to create a column using [flexbox layout](/guides/Flexible_box_layout).
