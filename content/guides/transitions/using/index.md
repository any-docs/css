...
CSS transitions let you decide which properties to animate (by [_listing them explicitly_](/reference/properties/transition-property)), when the animation will start (by setting a [_delay_](/reference/properties/transition-delay)), how long the transition will last (by setting a [_duration_](/reference/properties/transition-duration)), and how the transition will run (by defining an [_easing function_](/reference/properties/transition-timing-function), e.g., linearly or quick at the beginning, slow at the end).

## Which CSS properties can be transitioned?

The Web author can define which property has to be animated and in which way. This allows the creation of complex transitions. However, some properties are [not animatable](/guides/Animations/Animatable_properties) as it doesn't make sense to animate them.
