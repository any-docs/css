...
In this example, a glow effect is added using a [`drop-shadow()`](/reference/values/filter-function/drop-shadow) filter with a `3px` blur and `0` offset.
...
This example applies two filters — [`hue-rotate()`](/reference/values/filter-function/hue-rotate) and [`blur()`](/reference/values/filter-function/blur) — via the `backdrop-filter` property. The backdrop, the area behind the {{HTMLElement("p")}} element, has a color shift and a blur applied.
...
As filters are applied in sequential order, you can use filter functions more than once. In this example, the [`drop-shadow()`](/reference/values/filter-function/drop-shadow) filter has been used four times, each time with a different `<shadow>` value.
