...
In CSS, the **visual formatting model** describes how user agents take the document tree, and process and display it for visual media. This includes {{glossary("continuous media")}} such as a computer screen and [paged media](/guides/Paged_media) such as a book or document printed by browser print functions. Most of the information applies equally to continuous and paged media.
...
Other formatting contexts also create anonymous boxes. [Grid layout](/guides/Grid_layout) behaves in the same way as the [flexbox](/guides/Flexible_box_layout) example above, turning strings of text into a grid item with an anonymous box. [Multiple-column](/guides/Multicol_layout) layout creates anonymous column boxes around the columns; these also cannot be styled or otherwise targeted. [Table layout](/guides/Table) will add anonymous boxes to create a proper table structure — for example adding an anonymous table row — if there was no box with `display: table-row`.
...
Read more about [flow layout](/guides/Display/Flow_layout) in CSS.
...
Read about [CSS positioned layout](/guides/Positioned_layout).
...
**Independent formatting contexts** contain floats, and margins do not collapse across formatting context boundaries. Therefore, creating a new block formatting context can ensure that floats and margins remain inside a box. To do this, add `display: flow-root` to the box on which you wish to create a new [block formatting context](/guides/Display/Block_formatting_context).
...
- [CSS syntax](/guides/Syntax/Introduction) guide
