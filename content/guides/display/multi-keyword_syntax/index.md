...
The [CSS display module](/guides/Display) defines a multi-keyword syntax for the CSS `display` property. This guide explains the multi-keyword syntax.
...
One of the first things we learn about CSS is that some elements are block-level and some are inline-level. These are their [outer](/reference/values/display-outside) display types. For example, an `<h1>` or a `<p>` are block-level by default, and a `<span>` is inline-level. Using the `display` property we can switch between block and inline. For example to make a heading inline we would use the following CSS:
