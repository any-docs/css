...
Combinators, such as `+`, `&gt;`, `~`, [" "](/en-US/docs/Web/CSS/Reference/Selectors/Descendant_combinator), and `||`, may make a selector more specific in what is selected but they don't add any value to the specificity weight.
...
When creating complex selector lists with [CSS nesting](/guides/Nesting) this behaves in exactly the same way as the `:is()` pseudo-class.
...
CSS declarations marked as important override any other declarations within the same cascade layer and origin. Although technically, [`!important`](/reference/values/important) has nothing to do with specificity, it interacts directly with specificity and the cascade. It reverses the [cascade](/guides/Cascade/Introduction) order of stylesheets.
...
Including a ruleset inside a `@scope` block does not affect the specificity of its selector, regardless of the selectors used inside the [scope root and limit](/en-US/docs/Web/CSS/Reference/At-rules/@scope#syntax).
However, if you decide to explicitly add the `:scope` pseudo-class, you'll need to factor it in when calculating their specificity.
...
- [Introduction to CSS syntax: declarations, rulesets, and statements](/guides/Syntax/Introduction)
- [CSS error handling](/guides/Syntax/Error_handling)
- [At-rules](/guides/Syntax/At-rules)
- [Inheritance](/guides/Cascade/Inheritance)
- Values: [initial](/guides/Cascade/Property_value_processing#initial_value), [computed](/guides/Cascade/Property_value_processing#computed_value), [used](/guides/Cascade/Property_value_processing#used_value), and [actual](/guides/Cascade/Property_value_processing#actual_value)
