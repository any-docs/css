...
You can also create rectangles based on distances from the top and left edges of the reference box with the [`rect()`](/reference/values/basic-shape/rect) function, or by width and height with the [`xywh()`](/reference/values/basic-shape/xywh) function; both of these also support optional rounded corners.
...
An ellipse is a squashed circle. As such, the [`ellipse()`](/reference/values/basic-shape/ellipse) function acts in a very similar way to `circle()` except that we have to specify two radii, `x` and `y`, in that order.
...
The [`polygon()`](/reference/values/basic-shape/polygon) function is more complex, enabling the creation of multiple-sided polygon shapes. This shape accepts three or more pairs of values (a polygon must at least draw a triangle). Each space-separated pair of values is separated with a comma and represents the coordinates of a single vertex drawn relative to the reference box. Each pair of coordinates defines an edge of the polygon, with the final edge defined by the first and last set of coordinates.
...
To create even more complex shapes, you can define the outline of any shape with the [`path()`](/reference/values/basic-shape/path) or [`shape()`](/reference/values/basic-shape/shape) functions.
