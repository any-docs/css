...
The root element of a document is a stacking context which, in most cases, contains nested stacking contexts, many of which will contain additional stacking contexts. Within each stacking context, child elements are stacked according to the same rules explained in [Using `z-index`](/guides/Positioned_layout/Using_z-index). Importantly, the `z-index` values of its child stacking contexts only have meaning within its parent's stacking context. Stacking contexts are treated atomically as a single unit in the parent stacking context.
...
The CSS properties for colors, fonts, alignment, and [box-model](/guides/Box_model/Introduction) have been hidden for brevity.
...
Additional examples include a [2-level hierarchy with `z-index` on the last level](/guides/Positioned_layout/Stacking_context/Example_1), a [2-level HTML hierarchy, `z-index` on all levels](/guides/Positioned_layout/Stacking_context/Example_2), and a [3-level HTML hierarchy, `z-index` on the second level](/guides/Positioned_layout/Stacking_context/Example_3).
