...
The first article of this guide, [Stacking without the `z-index` property](/guides/Positioned_layout/Stacking_without_z-index), explains how stacking is arranged by default. If you want to create a custom stacking order, you can use the {{cssxref("z-index")}} property on a [positioned](/reference/properties/position#types_of_positioning) element.
...
> - If several elements share the same `z-index` value (i.e., they are placed on the same layer), stacking rules explained in the section [Stacking without the `z-index` property](/guides/Positioned_layout/Stacking_without_z-index) apply.
...
- [Stacking without `z-index`](/guides/Positioned_layout/Stacking_without_z-index)
