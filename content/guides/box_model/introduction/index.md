...
If the `box-sizing` property is set to `content-box` (default) and if the element is a block element, the content area's size can be explicitly defined with the `width`, `min-width`, `max-width`, {{ cssxref("height") }}, `min-height`, and `max-height` properties.
...
The size of the margin area is determined by the `margin-top`, `margin-right`, `margin-bottom`, `margin-left`, and shorthand `margin` properties. When [margin collapsing](/guides/Box_model/Margin_collapsing) occurs, the margin area is not clearly defined since margins are shared between boxes.
