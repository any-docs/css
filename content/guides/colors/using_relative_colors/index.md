...
In the section above we only saw relative colors defined via the [`rgb()`](/reference/values/color_value/rgb) function. However, relative colors can be defined using any modern CSS color function — [`color()`](/reference/values/color_value/color), [`hsl()`](/reference/values/color_value/hsl), [`hwb()`](/reference/values/color_value/hwb), [`lab()`](/reference/values/color_value/lab), [`lch()`](/reference/values/color_value/lch), [`oklab()`](/reference/values/color_value/oklab), [`oklch()`](/reference/values/color_value/oklch), or [`rgb()`](/reference/values/color_value/rgb). The general syntax structure is the same in each case, although the origin color values have different names appropriate for the function being used.
...
When creating a relative color, you can use values defined in [CSS custom properties](/guides/Cascading_variables/Using_custom_properties) both for the origin color and within the output color channel value definitions. Let's look at an example.
...
This example demonstrates changing the alpha channel of a named color. Here, we have an item wrapped in a container that both have a `teal` background. To distinguish between the backgrounds, we vary the alpha channel value using the relative color feature, the [`calc()` function](/reference/values/calc), and a [custom property](/reference/properties/--*).
...
> [!NOTE]
> You can find additional examples demonstrating the use of relative color syntax in the different functional notation types on their dedicated pages: [`color()`](/reference/values/color_value/color#using_relative_colors_with_color), [`hsl()`](/reference/values/color_value/hsl#using_relative_colors_with_hsl), [`hwb()`](/reference/values/color_value/hwb#using_relative_colors_with_hwb), [`lab()`](/reference/values/color_value/lab#using_relative_colors_with_lab), [`lch()`](/reference/values/color_value/lch#using_relative_colors_with_lch), [`oklab()`](/reference/values/color_value/oklab#using_relative_colors_with_oklab), [`oklch()`](/reference/values/color_value/oklch#using_relative_colors_with_oklch), [`rgb()`](/reference/values/color_value/rgb#using_relative_colors_with_rgb).
...
The colors themselves include the `--base-color`, plus relative colors derived from that `--base-color`. The relative colors use the [`lch()`](/reference/values/color_value/lch) function — passing in the origin `--base-color` and defining an output color with an adjusted lightness or hue channel as appropriate.
...
In the CSS the `:root` has a default `--hue` value set on it, relative [`lch()`](/reference/values/color_value/lch) colors to define the color scheme, plus a radial gradient that fills the whole body.
