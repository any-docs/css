...
The example below shows this overflow behavior. The multicol container has a set `height` and there is more text than space to create columns; therefore, we get columns created outside of the container.
...
In the example below, we used a `height` [@media query](/guides/Media_queries/Using) to ensure there is enough vertical space before applying the column properties.
...
In the final guide in this series, we will see [how fragmentation works with multicol layouts](/guides/Multicol_layout/Handling_content_breaks) to give us control over how content breaks between columns.
