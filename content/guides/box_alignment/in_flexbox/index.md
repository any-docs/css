...
The [box alignment](/guides/Box_alignment) module details how alignment works in various layout methods. In this guide, we explore how box alignment works in the context of [flexbox](/guides/Flexible_box_layout/Basic_concepts). As this guide aims to detail things which are specific to flexbox and box alignment, it should be read in conjunction with the [box alignment overview](/guides/Box_alignment/Overview) guide, which details the common features of box alignment across layout methods.

## Basic example

In this flexbox example, three flex items are aligned on the main axis using `justify-content` and on the cross axis using `align-items`. The first item overrides the `align-items` values set on the group by setting `align-self` to `center`.
...
- [Box alignment for block, absolutely positioned, and table layouts](/guides/Box_alignment/In_block_abspos_tables)
- [Aligning items in flex container](/guides/Flexible_box_layout/Aligning_items)
- {{Glossary("Cross axis")}}
- {{Glossary("Main axis")}}
