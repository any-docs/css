...
Container queries are an alternative to [media queries](/guides/Media_queries), which apply styles to elements based on viewport size or other device characteristics.

This article provides an introduction to using container queries, specifically focusing on size container queries. Other guides discuss [style](/guides/Containment/Container_size_and_style_queries#container_style_queries) and [scroll-state](/guides/Conditional_rules/Container_scroll-state_queries) container queries in detail.
...
- `size`
  - The query will be based on the [inline and block](/guides/Logical_properties_and_values/Basic_concepts#block_and_inline_dimensions) dimensions of the container.
    Applies layout, style, and size [containment](/guides/Containment/Using) to the container.
- `inline-size`
  - The query will be based on the [inline](/guides/Logical_properties_and_values/Basic_concepts#block_and_inline_dimensions) dimensions of the container.
...
If no eligible container is available for the query, the container query length unit defaults to the [small viewport unit](/reference/values/length#small_viewport_units) for that axis (`sv*`).
...
For more information on these units, see the [Container query length units](/reference/values/length#container_query_length_units) reference.
