...
CSS custom functions are defined in `@function` at-rules and called using `&lt;dashed-function>` syntax, which looks very similar to [CSS custom property](/reference/properties/--*) syntax except that it includes parentheses at the end containing arguments (for example, `--my-function(30px, 3)`). CSS custom functions can be called within any property value, and will return a value based on the arguments passed into the function and the logic inside it.
